...
function showCalendar(month,year) {
    ...
    // create <div> calendar
    createElement("DIV", "contain", "calendar", nothing);
    ...
    document.getElementById("button3").value = month; // set value for select month
    document.getElementById("button4").value = year; // set value for select year
    if (month < 10) {
        month = "0" + month;
    }
    var a = (year + "-" + month + "-01");
...
function createDate(first, last, nameParent, month, year) {
    ...
    console.log(firstDayOfMonth,endDayOfMonth)
    for (var i = first; i <= last; i++) { // Set content for date in week
        if (nameParent == "date") {
            ...
            if (flag == (currentDate.getDate() + 1) 
                ...
                createElement("button", nameParent, "button1", containOption);
            }
        }
    }
}
